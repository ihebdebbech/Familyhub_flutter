...
import 'package:flutter_application_1/flame_game/components/background.dart';
...
class EndlessWorld extends World with TapCallbacks, HasGameReference {
  ...
  late double speed = _calculateSpeed(level.number.toDouble());
  ...
  late final Teleportation teleportation;
  ...
  Future<void> onLoad() async {
    ...
    teleportation = Teleportation(teleportpos: Vector2(size.x, groundLevel));
    ...
    if (level.number == 1) {
      add(
        ...
        add(
        SpawnComponent(
          factory: (_) => Obstaclelevel2(teleportpos: Vector2(size.x, groundLevel+40)),
          period: 5,
          area: Rectangle.fromPoints(
            Vector2(size.x / 2, groundLevel),
            Vector2(size.x / 2, groundLevel),
    ...
    add(
      SpawnComponent.periodRange(
        factory: (_) => floatingground.random(
          random: _random,
          canSpawnTall: level.canSpawnTall,
        ),
        minPeriod: 3.0,
        maxPeriod: 7.0,
        area: Rectangle.fromPoints(
          Vector2(size.x / 2, groundLevel - 300),
          Vector2(size.x / 2 + 100, groundLevel - 300),
        ),
        random: Random(),
      ),
    );
    ...
    scoreNotifier.addListener(() {
      if (scoreNotifier.value >= level.winScore) {
        ...
        //level = gameLevels[level.number];
        //speed = _calculateSpeed(level.number + 0.2);
        add(teleportation);
        // game.camera.backdrop.removeAll(game.camera.backdrop.children);
        // game.camera.backdrop.add(Background(speed: speed, level: level));
        // player.level = gameLevels[level.number];
        // player.onLoad();
  ...
  static double _calculateSpeed(double level) => 200 + (level * 200);
}
